...
class _HomeFolderScreenState extends State<HomeFolderScreen> {
  ...
  Future<void> _processWebImage(Uint8List bytes) async {
    final uri = Uri.parse("http://192.168.75.109:5000/predict");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final resString =
            await response.stream.bytesToString(); // <-- Important
        final decoded = json.decode(resString);
        String suggestedFolder = decoded['prediction'];
      ...
      debugPrint("Web classification error: $e");
    }
  }

  Future<String> _classifyImage(File image) async {
    var uri = Uri.parse("http://192.168.75.109:5000/predict");
                      ...
                      ElevatedButton(
                        onPressed: () {
                          if (kIsWeb && _webImageBytes != null) {
                            _processWebImage(_webImageBytes!);
                          } else if (image != null) {
                            _processImage(image!);
                          }
                        },
